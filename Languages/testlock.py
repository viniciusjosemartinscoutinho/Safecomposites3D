...
def descobrir_melhor_pasta():
    ...
    pasta_local = os.path.join(os.environ['LOCALAPPDATA'], f"{folder_name}")
    
    # 2ª Opção: AppData Roaming (Excelente esconderijo e alta compatibilidade)
    pasta_roaming = os.path.join(os.environ['APPDATA'], f"{folder_name}")
    
    # 3ª Opção: Zona Neutra Temporária (O plano de emergência)
    pasta_temp = f"C:\\Temp\\{folder_name}" 
